---
name: create
description: Scaffold a new mindframe dashboard for the user's stack. Interview the user about what they want to watch, probe their environment for what tools are available, build a systems.json + dashboard.py in a sibling directory, seed the right packs, and boot a live dashboard. Use when asked to "create a dashboard", "build me a status page", "set up mindframe", or when the user describes infrastructure they want to monitor.
---
...
- `dashboard.py` — 4-line FastAPI entry point that calls `mindframe.serve(...)`
- `packs/` (optional) — source packs you generate for platforms the user uses

The mindframe plugin ships the server factory, built-in primitives (`http`, `tcp`, `cmd`, `file`), a threshold engine, and reference packs. You compose these into a dashboard tailored to the user.
...
Create the dashboard directory as a sibling of the mindframe plugin, or wherever the user specified.

```
<name>-dashboard/
├── dashboard.py          # imports mindframe, calls serve()
├── systems.json          # the user's systems + components
├── requirements.txt      # mindframe + uvicorn
├── .gitignore
...
For each new pack, pass control to `/mindframe:dev` or run its steps inline:

1. **Capture a real fixture** using the user's credentials (`gh api`, `kubectl get`, `curl -H "Authorization: Bearer …"`). Write to `packs/<platform>/fixtures/<label>.json` using `mindframe.fixtures.capture()`.
...
Create a venv in the dashboard directory. Install mindframe as an editable dependency:

    python3 -m venv .venv
    .venv/bin/pip install -e <path-to-plugin>/mindframe
...
### 8. Commit and hand back
    ...
    git -C <dashboard-dir> commit -m "scaffold mindframe dashboard"
...
Hand off to `/mindframe:dev` for further iteration.
...
- **Do not add dependencies beyond mindframe and uvicorn** unless the user asked for them. Keep the scaffold minimal.
